...
class ThumbnailMaker(BasePlugin):
    ...
    def on_page_content(self, html, **kwargs):
        ...
        for link in links:
            ...
            thumbnail.createPdfThumb(
                srcDir / Path(filename), tgtDir / Path(filename + "-thumb.png")
            )
        ...
        for link in links:
            ...
            try:
                if "playlist" in href:
                    id = re.search(r".*playlist\?list=([0-9A-Za-z_-]+).*", href).group(
                        1
                    )
                    thumbnail.createYouTubePlaylistThumb(
                        href, tgtDir / Path(id + "-thumb.png")
                    )
                else:
